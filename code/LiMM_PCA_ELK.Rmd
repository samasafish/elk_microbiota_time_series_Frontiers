---
title: "LiMM-PCA with Elk time series data"
author: "Sam Pannoni"
...
data_path <- "r_output/intermediate_rds_files"
DirectoryFun <- file.path("Manon_LIB/R_Codes/4-ASCA_globalmethods/LIB")
DirectoryFun_LiMM <- file.path("Manon_LIB/R_Codes/5-LiMMPCA/LIB")
fig_path <- "r_output/visualizations/LIMM"
# out_path <- "outputs"
getwd()

Vers <-  format(Sys.time(), "%b_%d.%Y")
# Vers <- "without45_"
...
# (volunteer, blood sampling for a given volunteer, blood tube for a
...
knitr::opts_chunk$set(echo = TRUE, dev = c('png'))


# Source code for LiMMPCA, functions dimreducPCA() and parlmer()
source(file.path(DirectoryFun_LiMM, "dimreducPCA.R"))
source(file.path(DirectoryFun_LiMM, "parlmer_interaction.R"))
source(file.path(DirectoryFun_LiMM, "parlmer.R"))
# Colors +++++
...
# Dimension reduction by PCA {.tabset}
Scree plot
...
a <- grid.arrange(screeplotTime, Time_scores,
             ...
             top=textGrob("Day (of sampling) effect matrix",
...
c <- grid.arrange(screeplotSampling,
              ...
              nrow=1,widths=c(0.3, 1,  1),
             top=textGrob("Sample Replication effect matrix",
                          gp=gpar(fontsize=20,font=2)))
...
plots combined
```{r, fig.heigth = 120, fig.width=14, eval=T}
...
names(pval) <- c("Elk","Replicate", "Day")
pander("p-values")
pander(pval)
...
df_rename <- difmat %>% mutate(Effect = as.factor(Effect)) %>% 
...
df_rename$PC <- factor(df_rename$PC, levels = as.character(c(1:40)))

class(df_rename$PC)
# difmat$Effect

LLRplot <- ggplot(data=df_rename, aes(x=PC, y=value, fill = Effect)) +
